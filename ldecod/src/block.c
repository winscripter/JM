...
void iTransform(Macroblock* currMB, ColorPlane pl, int smb)
{
	...
	if ((dec_picture->chroma_format_idc != YUV400) && (dec_picture->chroma_format_idc != YUV444))
	{
		...
		{
			imgpel** curUV = &dec_picture->imgUV[uv - 1][currMB->pix_c_y];
			imgpel** mb_rec = currSlice->mb_rec[uv];
			...
			else if (smb)
			{
				...
				for (int joff = 0; joff < p_Vid->mb_cr_size_y; joff += BLOCK_SIZE)
				{
					for (int ioff = 0; ioff < p_Vid->mb_cr_size_x;ioff += BLOCK_SIZE)
					{
...
int CheckVertMV(Macroblock* currMB, int vec1_y, int block_size_y)
{
	...
	return y_pos < (-p_Vid->iLumaPadY + 2) || y_pos >(maxold_y + p_Vid->iLumaPadY - block_size_y - 2);
}
